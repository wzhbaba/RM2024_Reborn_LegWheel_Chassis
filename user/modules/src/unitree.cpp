 ...
 *
 * This function is used to initialize the Unitree Motor with the specified UART handle, ID, and mode.
 *
 * @param _p_huart Pointer to the UART handle.
 * @param _id The ID of the motor.
 * @param _mode The mode of the motor.
 */
void Unitree_Motor::Init(UART_HandleTypeDef *_p_huart, uint8_t _id, uint8_t _mode)
{
    p_huart_ = _p_huart;
    id_ = _id;
    mode_ = _mode;
 ...
 */
void Unitree_Motor::SetMotorData(float _Pos, float _T, float _W, float _K_P, float _K_W)
{
    motor_send_.id = id_;
...
int Unitree_Motor::SendData()
{
    ...
    motor_send_.ComData.CRC32 = Get_CRC32_Check_Sum((uint32_t *)&motor_send_.ComData, 7);
 ...
 *
 * This function is responsible for updating the Unitree Motor based on the provided data.
 *
...
{
    if (motor_recv_.ServoData.CRCdata == Get_CRC32_Check_Sum((uint32_t *)(pData), 18)) {
        ...
        motor_recv_.W = ((float)motor_recv_.ServoData.Mdata.W / 128) * 0.95f + motor_recv_.W * 0.05f;
        motor_recv_.T = (float)motor_recv_.ServoData.Mdata.T / 256;
        motor_recv_.Pos = 6.2832f * ((float)motor_recv_.ServoData.Mdata.Pos) / 16384;
    }
    return 0;
}
...
void Unitree_Motor::Ctrl()
{
    ...
    HAL_UART_Transmit(p_huart_, (uint8_t *)&motor_send_.ComData, sizeof(motor_send_.ComData), 10);
    HAL_GPIO_WritePin(p_port_, pin_, GPIO_PIN_RESET);
    HAL_UARTEx_ReceiveToIdle_DMA(p_huart_, (uint8_t *)&motor_recv_.ServoData, sizeof(motor_recv_.ServoData));
